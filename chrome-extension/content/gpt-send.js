...
window.__iaExtensionGetGptReady = function () {
  if (window.__iaWpfPrepInFlight) {
  ...
  if (__iaIsStopGeneratingControlVisible()) {
    return { ok: false, reason: "generating" };
  }
  return { ok: true };
};

/** Chrome extension: send prompt into real ChatGPT composer (no WebView2). */
window.__iaExtensionSendPrompt = function (userPrompt, appendToExisting) {
